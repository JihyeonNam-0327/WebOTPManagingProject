...
<%@ page import="java.sql.*, javax.sql.*, java.io.*, java.util.*, java.math.*, java.text.*, java.net.*" %>
...
<%!
	...
	private String traverseDate(String type, int year, int month){
		...
		if(type.equals("upYear")){
			href = "./calendarTest.jsp?year=" + (year+1) + "&month=" + month;
		}else if(type.equals("downYear")){
			href = "./calendarTest.jsp?year=" + (year-1) + "&month=" + month;
		}else if(type.equals("upMonth")){
			if(month == 11){
				href = "./calendarTest.jsp?year=" + (year+1) + "&month=0";
			}else{
				href = "./calendarTest.jsp?year=" + year + "&month=" + (month+1);
			}
		}else if(type.equals("downMonth")){
			if(month == 0){
				href = "./calendarTest.jsp?year=" + (year-1) + "&month=11";
			}else{
				href = "./calendarTest.jsp?year=" + year + "&month=" + (month-1);
			}
		}
...
try{
	...
	out.println("		<a href='"+traverseDate("upYear", year, month)+"'><font style='text-decoration: none;'> &nbsp;〉</font></a>&nbsp;");
	out.println("		<a href='"+traverseDate("downMonth", year, month)+"'><font style='text-decoration: none;'>〈&nbsp; </font></a>");
	out.println("		<font style='font-size: 18'>"+(month+1)+"</font><a href='"+traverseDate("upMonth", year, month)+"'>");
	...
	for(int i = 0; i < col; i++){
		...
		for(int j = 0; j < row; j++){
			out.println("<td class=mytable width=160 bgcolor='#f8f8f8' valign=top>");
			if(day <= lastDay){
